...
class DoctorPatientFileSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'id', 'doctor', 'patient', 'age', 'sex', 'cp', 'trestbps', 'chol', 'fbs', 'restecg', 'thalach', 'exang', 'oldpeak', 'slope', 'ca', 'thal',  'rf_diagnosis',
            'dat_file', 'hea_file', 'xyz_file', 'image_ii', 'image_v6', 'image_vz', 'ecg_diagnosis',
            
            'video', 'echo_diagnosis', 'prognosis', 'final_diagnosis', 'created_at'
        ]
        read_only_fields = ['id', 'created_at']
...
class ECGFileUploadSerializer(serializers.Serializer):
    dat_file = serializers.FileField()
    hea_file = serializers.FileField()
    xyz_file = serializers.FileField(required=False)


class EchoFileUploadSerializer(serializers.Serializer):
