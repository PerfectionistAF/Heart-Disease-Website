    ...
    const handleOptionClick = async(option) => {
        ...
        switch(option) {
            ...
            case 'FSeverity asc':
                ...
                const response5 = await axios.get(`${process.env.REACT_APP_API_URL}doctor-patient-files/?doctor=${user?.id}&diagnosis=present`);
                console.log(response5.data);
                const len = response5.data.length;
                const files = response5.data;
                const newPatients = [];
            ...
            case 'FSeverity des':
                ...
                const response7 = await axios.get(`${process.env.REACT_APP_API_URL}doctor-patient-files/?doctor=${user?.id}&diagnosis=absent`);
